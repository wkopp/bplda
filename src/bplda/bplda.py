...
from bplda._bplda import _loglikelihood
...
class LDA(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, X):
        ...
        Returns
        -------
        doc_topics : np.array[n_docs, n_topics]
    ...
    def score(self, X):
        ...
        assert X.shape[0] == self.word_topic_.shape[0]
        assert X.shape[1] == self.doc_topic_.shape[0]

        return _loglikelihood(
            w, d, x.reshape(-1).astype("float"), self.doc_topic_, self.word_topic_
        )
